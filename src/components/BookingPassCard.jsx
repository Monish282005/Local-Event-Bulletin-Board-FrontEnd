...
export default function BookingPassCard({ booking, onBookingCancelled }) {
  const { event, total_user_tickets, ticket_numbers, booked_at } = booking;
  ...
  return (
    <div className="bg-white border border-[#E8E7EF] hover:border-[#5B4BFF]/40 rounded-3xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden flex flex-col justify-between group">
      ...
      <div className="p-6 bg-[#FAF9FE] space-y-4">
        ...
        {/* Cancellation Action & Policy Status */}
        <div className="pt-2 border-t border-[#E8E7EF]/80 flex items-center justify-between gap-2">
          {canCancel ? (
            <button
              onClick={handleCancelBooking}
              disabled={cancelling}
              className="w-full py-2 rounded-xl bg-white hover:bg-red-50 text-red-600 hover:text-red-700 border border-red-200 font-bold text-xs transition flex items-center justify-center gap-1.5 shadow-xs disabled:opacity-50"
            >
              <span>🗑️</span>
              <span>{cancelling ? 'Cancelling Pass...' : 'Cancel Booking'}</span>
            </button>
          ) : (
            <div
              className="w-full py-2 rounded-xl bg-slate-100 text-slate-500 border border-slate-200 font-bold text-xs text-center flex items-center justify-center gap-1"
              title="The organizer has marked tickets for this event as non-cancellable."
            >
              <span>🚫</span>
              <span>Non-Cancellable Pass</span>
            </div>
          )}
        </div>
      </div>
